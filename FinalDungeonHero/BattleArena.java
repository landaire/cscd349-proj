/*
	This is the BattleArena class which contains the main. In the previous assignment we couldn't get our dungeon to work right
	so we decided to just do a battle between the hero that is choosen and random monsters. Even though there was no dungeon
	we learned a lot about inheritence and polymorphism. On the other hand, this displays everything that is going on.
  
  Extra Credit: Extra Hero was added. 
  					 Design was well put. 
					 This is easy to run. First time users will not get lost == bonus

  History:
  5/3/2012 - started to build the classes need for the main
  5/4/2012 - started on the main
  5/6/2012 - finished with it running without the dungeon
  

  @author  Tony Moua and Daniel Moua
  @see     no see
*/
...
/**
	...
	public DungeonCharacter chooseHero()
	{
		...
		while (choice.charAt(0) < 'a' || choice.charAt(0) > 'd');				
		switch (choice.charAt(0))
			{
				...
				case 'd': System.out.println("You have choosen Sora"); //warrior choice
						  return new Warrior("Sora",350, 6,.90,85,105,.60);
				default:
...
/**
	...
	public DungeonCharacter randomMonster()
	{
		...
		if(num == 1)
		{
			...
			return new Ogre("Ogre", 100,2,.60,30,60,.10,30,40);
		}
		else if(num == 2)
		{
			System.out.println("You are fighting an Gremilin" + "\n");
			return new Gremlin("Pete", 50,5,.80,15,30,.40,20,40);
		} 
		else
			System.out.println("You are fighting an Skeleton" + "\n");
			return new Skeleton("Bones", 75,3,.80,30,50,.30,30,50);
